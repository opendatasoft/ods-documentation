...
This connector creates records from an arbitrary XML structure by converting all elements at a specific depth (optionally filtered by tag) to a set of records.
...
In this example, the XML tree is complex. As a result, the automatic parameters detection cannot guess the proper depth. You must configure the XML connector manually:
